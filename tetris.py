...
def init():
    global screen, clock, running, dt, nextPieces, nextColors
    print("Game Started")
    ...
    nextColors = list(range(len(colors)))
...
def spawnBlock():
    global nextPieces, nextColors, currBlock, currColor, rotation, pos
       
    currBlock = blocks[nextPieces.pop(0)]
    currColor = colors[nextColors.pop(0)]
...
def gameLoop():
    ...
    while running:
        ...
        for i in range(4):
            ...
            pygame.draw.rect(screen, currColor, pygame.Rect(topleft.x+(pos.x+rotatedBlock[i].x)*32, topleft.y + (pos.y + rotatedBlock[i].y)*32, 32, 32))
        ...
        for i in range(4):
            if nextPieces:
                ...
                draw_rect_alpha(screen, colors[nextColors[0]], pygame.Rect(topleft.x+playArea.x*32 + 96 + rotatedBlock[i].x*32, topleft.y - 16 + rotatedBlock[i].y*32, 32, 32), 0)
